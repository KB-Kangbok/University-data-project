...
ggplot(data = College_tbl_data) +
  ...
  geom_line(aes(x = adm_group, y = Avg_earning / 10000), color = "red", size = 1.1) +
  theme_bw() +
  ylab("Modified average tuition & salary") +
  xlab("Admission rates quotient by 10") +
  geom_text(x = 7, y = 20000, label = "Salary line", color = "red") +
  geom_text(x = 1, y = 20000, label = "Tuition line", color = "steelblue") +
...
CollegesSummary <- College_final %>%
  ...
  mutate(ROI = Average_earning - Mean_Cost_of_Attendance * 4) %>%
  rename(
    Region = REGION
  )
...
ggplot(data = CollegesSummary) +
  geom_bar(aes(x = factor(Region), y = ROI / 10^8), stat = "identity", fill = "steelblue") +
  theme_bw() +
